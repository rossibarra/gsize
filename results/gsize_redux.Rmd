...
Go back to RIMMAs because Paul's script can't handle hapmap2. Mapped RIMMAs to v3 version 22 of the cDNA with only "known" cDNAs, then counted abundance only of genes with proportion read mapping < 5E-5

Get data
```{r}
#genome size
rimma_gs<-read.csv("data/gsize_rimma.csv",header=T)
#percent mapping
rimma_perc<-read.csv(""~/Desktop/"results/fixed_genes_precent.txt",header=T)
rimma_data<-merge(rimma_gs,rimma_perc,by.x="Line",by.y="file")
```

Correlation and plot -- WOO DADDY!
```{r}
